...
import { findHeadToHeadMatches } from './utils.js';
import { 
    displayStandings, 
    displayFixtures,
...
const DOM = {
    ...
    kings: { select: document.getElementById('seasonSelectKings'), scorersContainer: document.getElementById('top-scorers'), assistsContainer: document.getElementById('top-assists'), cleanSheetsContainer: document.getElementById('top-cleansheets') },
...
function handleH2HSelectionChange() {
    ...
    // Her bir fikstürün yanına hangi sezona ait olduğunu ekliyoruz.
    const allFixturesWithSeason = Object.entries(AppState.seasons).flatMap(([seasonId, seasonData]) => 
        seasonData.fixtures.map(fixture => ({ ...fixture, seasonId }))
    );
    ...
    // Filtrelenmiş maçları buluyoruz.
    const matches = findHeadToHeadMatches(team1Id, team2Id, allFixturesWithSeason);
    
    // Sonuçları gösteren fonksiyona, maçları ve tüm sezon verilerini gönderiyoruz.
